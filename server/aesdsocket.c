...
void *timestamp_thread(void *arg){
    ...
    while(!stop_server){
        ...
        pthread_mutex_lock(&file_mutex);
        int file_fd = open(STORAGE_PATH, O_WRONLY | O_CREAT | O_APPEND, 0644);
        if(file_fd >= 0){
            write(file_fd, ts_buf, strlen(ts_buf));
            close(file_fd);
        } 
        else{
            syslog(LOG_ERR, "Failed to open %s for timestamp: %s", STORAGE_PATH, strerror(errno));
        }
        pthread_mutex_unlock(&file_mutex);
    }
    return NULL;
...
#if !USE_AESD_CHAR_DEVICE
    if(unlink(STORAGE_PATH) < 0)
        syslog(LOG_ERR, "Failed to delete %s: %s", STORAGE_PATH, strerror(errno));
    else
        syslog(LOG_INFO, "Deleted %s", STORAGE_PATH);
#endif
